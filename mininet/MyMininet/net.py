...
class Mininet( object ):
    ...
    ****Wirtten by ZHL
    '''
    #get the links that to be deleted
    ...
    #get the nodes that to be deleted
    def getNodesToDelete(self, filePath):
      nodes = []
      for line in fileinput.input(filePath):
        info(line + '\n')
        #remove the line-break(compatible OS: Windows and Linux)
    ...
    def selfDeleteLinks(self, linksFilePath):
      ...
      for element in links:
        ...
        if(self.linksBetween(node1 , node2)):
          ...
          info('***link (' + element[0] + ',' + element[1] + ') delete finished!' + '\n')
        else:
          info('***link (' + element[0] + ',' + element[1] + ') not exists!' + '\n')

    def selfPing( self, hosts=None, timeout=None ):
        ...
        ploss = None
        hosts = self.switches
        output( '*** selfPing: testing ping reachability\n' )
        for node in hosts:
            output( '%s -> ' % node.name )
            for dest in hosts:
                if node != dest:
    ...
    def selfDeleteNodes(self, nodesFilePath):
      info('***delete serval nodes in one time' + '\n')
      info('***file position : ' + nodesFilePath + '\n')
	  ...
	  elementName = list(self.nameToNode.keys())[pos]
          ...
          if(tempLink):
            info('*** link(' +node + '<->' + elementName  + ')  will to be deleted!' + '\n')
            self.delLinkBetween(self.nameToNode[node]  , element)
          else:
            pass
      for node in nodes:
	hostName = node.replace('s','h')
	info('*** link(' +node + '<->' + hostName  + ')  will to be deleted!' + '\n')
    ...
    def iperfAll(self , l4Type='TCP', udpBw='10M', fmt=None):
      output( '*** Iperf: testing', l4Type, 'bandwidth' +'\n' )
        ...
        for j in range(length - i):
          ...
          if target == i:
            ...
            if self.linksBetween(nodeLeft , nodeRight):
              res = self.iperfWithoutInfo([self.hosts[i],self.hosts[target]])
	      client = list(self.nameToNode.keys())[list(self.nameToNode.values()).index(self.hosts[i])]
	      server = list(self.nameToNode.keys())[list(self.nameToNode.values()).index(self.hosts[target])]
              output('*** ' + l4Type + ' bandwidth between ' + client  +' and '+ server +':')
	      output(res)
	      output('\n')

    def iperfWithoutInfo( self, hosts=None, l4Type='TCP', udpBw='10M', fmt=None,
               seconds=5, port=5001):
